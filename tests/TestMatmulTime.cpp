...
/** CoordList matmul benchmark functions **/

void benchmark_CoordList_matmul_naive(int M, int K, int N, double sparsity,
...
void benchmark_CoordList_matmul_optimized(int M, int K, int N, double sparsity,
                                const std::string &label, bool csv = false) {
  // std::random_device rd;
  int seedA = 1;
  int seedB = 2;
...
/** CSR matmul benchmark functions **/

void benchmark_CSR_matmul_naive(int M, int K, int N, double sparsity,
                            const std::string &label, bool csv = false) {
  std::random_device rd;
  int seedA = rd();
  int seedB = rd();
...
void benchmark_CSR_matmul_optimized(int M, int K, int N, double sparsity,
                                const std::string &label, bool csv = false) {
  // std::random_device rd;
  int seedA = 1;
  int seedB = 2;
...
/** CoordList matmul TESTS **/

TEST_CASE("CoordList Performance Benchmark, (single op, naive)",
          "[CoordListMatrix]") {
  SECTION("Single matmul operation, small naive") {
    benchmark_CoordList_matmul_naive(1000, 800, 1000, 0.005, "[naive matmul, small]");
  }

  SECTION("Single matmul operation, medium naive") {
    benchmark_CoordList_matmul_naive(10000, 20000, 20000, 0.005,
                           "[naive matmul, medium]");
  }

  SECTION("Single matmul operation, large naive") {
    benchmark_CoordList_matmul_naive(300000, 300000, 3000000, 0.0005,
                           "[naive matmul, large]");
  }
}
...
TEST_CASE("CoordList Scaling Sweep (single op, naive)", "[benchmark]") {
  ...
  for (int N = start_N; N <= end_N; N += step) {
    ...
    benchmark_CoordList_matmul_naive(M, K, N, sparsity, label, true);
  }
}
...
TEST_CASE("CoordList Scaling Sweep (single op, optimized)", "[benchmark]") {
  ...
  for (int N = start_N; N <= end_N; N += step) {
    ...
    std::string label = "[naive-sweep N=" + std::to_string(N) + "]";
    benchmark_CoordList_matmul_optimized(M, K, N, sparsity, label, true);
  }
}
...
TEST_CASE("CSR Performance Benchmark, (single op, naive)",
          "[CSRMatrix]") {
  SECTION("Single matmul operation, small naive") {
    benchmark_CSR_matmul_naive(1000, 800, 1000, 0.005, "[naive matmul, small]");
  }

  SECTION("Single matmul operation, medium naive") {
    benchmark_CSR_matmul_naive(10000, 20000, 20000, 0.005,
                           "[naive matmul, medium]");
  }

  SECTION("Single matmul operation, large naive") {
    benchmark_CSR_matmul_naive(300000, 300000, 3000000, 0.0005,
                           "[naive matmul, large]");
  }
}
...
TEST_CASE("CSR Scaling Sweep (single op, naive)", "[benchmark]") {
  ...
  for (int N = start_N; N <= end_N; N += step) {
    ...
    std::string label = "[naive-sweep N=" + std::to_string(N) + "]";
    benchmark_CSR_matmul_naive(M, K, N, sparsity, label, true);
  }
}
...
TEST_CASE("CSR Scaling Sweep (single op, optimized)", "[benchmark]") {
  ...
  for (int N = start_N; N <= end_N; N += step) {
    ...
    std::string label = "[naive-sweep N=" + std::to_string(N) + "]";
    benchmark_CSR_matmul_optimized(M, K, N, sparsity, label, true);
  }
}
